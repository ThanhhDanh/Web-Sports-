<div class="container-fluid px-4">
    <div class="container bg-container">
        <h1 class="mb-5 ps-4 name-create-invoice">Chỉnh sửa hóa đơn</h1>
        <form wire:submit.prevent="updateInvoice">
                ...
                <table class="table table-borderless text-center">
                    <thead>
                        <tr class="table-primary">
                            ...
                            <th style="width: 150px" scope="col">Màu</th>
                            <th style="width: 180px" scope="col">Size</th>
                    ...
                    <tbody>
                        @foreach ($selectedProducts as $id => $product)
                            <tr>
                                ...
                                <td class="p-10">{{ $product['quantity'] }}</td>
                                <td class="p-10">
                                    <select wire:model="selectedColors.{{ $id }}"
                                        wire:change.prevent="updateColor($event.target.value, {{ $id }})"
                                        id="color-{{ $id }}" name="color" class="form-select"
                                        aria-label="Default select example">
                                        <option value="">-- Chọn màu --</option>
                                        @foreach ($product['colors'] as $color)
                                            <option value="{{ $color['id'] }}"
                                                @if ($color['id'] == $product['color']) selected @endif>
                                                {{ $color['name'] }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <td class="p-10">
                                    <select wire:model="selectedSizes.{{ $id }}"
                                        wire:change.prevent="updateSize($event.target.value, {{ $id }})"
                                        id="size-{{ $id }}" name="size" class="form-select"
                                        aria-label="Default select example">
                                        <option value="">-- Chọn kích thước --</option>
                                        @foreach ($product['sizes'] as $size)
                                            <option value="{{ $size['id'] }}"
                                                @if ($size['id'] == $product['size']) selected @endif>
                                                {{ $size['name'] }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <td class="p-10">0 VNĐ</td>
                                <td class="p-10">{{ number_format($product['tax']) }}%</td>
                                <td class="p-10">
                                    {{ number_format($product['price'] * (1 + $product['tax'] / 100)) }}
                                    VNĐ
                                </td>
                                <td>
            ...
            <div style="border-top: 1px solid rgb(232 231 231)" class="row ps-4 mt-5">
                <div class="col-xl-4 col-md-6 mt-4">
                    <div class="form-group mb-3">
                        ...
                        <div class="form-input">
                            <select wire:model="selectedDiscount"
                                wire:change.prevent="addDiscount($event.target.value)" id="discount" name="discount"
                                class="form-select" aria-label="Default select example">
                    ...
                    <div class="col-right">
                        ...
                        <div class="col-item d-flex align-items-center justify-content-between">
                            ...
                            <span class="money-item">
                                {{ number_format($this->selectedDiscountPrice, 0, '.', ',') }} VNĐ
                            </span>
                        </div>
                        <div class="col-item d-flex align-items-center justify-content-between">
                            <span class="label-item mb-3 money-bill">Tổng hóa đơn</span>
                            <span class="money-item">{{ number_format($totalAmount, 0, '.', ',') }} VNĐ</span>
                        </div>
            ...
            <div class="d-flex justify-content-end pe-4 pb-5">
